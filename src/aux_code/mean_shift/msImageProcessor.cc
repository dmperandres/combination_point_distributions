...
msImageProcessor::msImageProcessor( void )
{
  ...
  //intialize visit table to having nullptr entries
  visitTable			= nullptr;
...
void msImageProcessor::DefineImage(byte *data_, imageType type, int height_, int width_)
{
	...
	if(!h)
	{
		...
		_tdef_ns::kernelType	k[2]		= {_tdef_ns::Uniform,_tdef_ns::Uniform};
		int			P[2]		= {2, N};
...
void msImageProcessor::Filter(int sigmaS, float sigmaR, _tdef_ns::SpeedUpLevel speedUpLevel)
{
...
void msImageProcessor::Segment(int sigmaS, float sigmaR, int minRegion, _tdef_ns::SpeedUpLevel speedUpLevel)
{
	...
	if(ErrorStatus == _tdef_ns::EL_HALT)
		return;
...
void msImageProcessor::LUVtoRGB(float *luvVal, byte *rgbVal)
{
  ...
  if(luvVal[0] < 0.1f)
	...
	else
	{
		//convert luv to xyz...
    if(luvVal[0] < 8.0f)
      y	= Yn * luvVal[0] / 903.3f;
		else
		{
      y	= (luvVal[0] + 16.0f) / 116.0f;
			y  *= Yn * y * y;
		}
...
{
  //make sure that outputImageData is not nullptr
	if(!outputImageData)
	{
    ErrorHandler("msImageProcessor", "GetRawData", "Output image data buffer is nullptr.");
		return;
	}
...
void msImageProcessor::GetResults(byte *outputImage)
{
  ...
  //make sure that outpuImage is not nullptr
	if(!outputImage)
	{
    ErrorHandler("msImageProcessor", "GetResults", "Output image buffer is nullptr.");
		return;
	}
...
/*      - nullptr is returned if the image has not been   */
...
void msImageProcessor::BuildRAM( void )
{
  ...
  raPool[NODE_MULTIPLE*regionCount-1].next	= nullptr;
