...
void MeanShift::DefineKernel(_tdef_ns::kernelType *kernel_, float *h_, int *P_, int kp_)
{
	...
	if((!(P = new int [kp]))||(!(h = new float [kp]))||(!(kernel = new _tdef_ns::kernelType [kp]))||
	...
	//check for errors
	if(ErrorStatus == _tdef_ns::EL_ERROR)
		return;
...
//  //make sure x is not nullptr...
//	if(!x)
//	{
//    ErrorHandler("MeanShift", "UploadInput", "Input data set is nullptr.");
//		return;
//	}
...
{
  //make sure wm is not nullptr
	if(!wm)
	{
    ErrorHandler("MeanShift", "SetWeightMap", "Specified weight map is nullptr.");
		return;
	}
...
void MeanShift::msVector(double *Mh, double *yk)
{
  ...
  //make sure that Mh and/or yk are not nullptr...
	if((!Mh)||(!yk))
	{
...
void MeanShift::FindMode(double *mode, double *yk)
{
  ...
  //make sure that mode and/or yk are not nullptr...
	if((!mode)||(!yk))
	{
...
void MeanShift::ErrorHandler(const char *className,const char *methodName,const char* errmsg)
{
	...
	strcpy((char *)ErrorMessage, (char *) className);
	strcat((char *)ErrorMessage, "::");
	strcat((char *)ErrorMessage,(char *)methodName);
	strcat((char *)ErrorMessage, " Error: ");
	
	//store message into error message
	strcat((char *)ErrorMessage,(char *) errmsg);
	
	//set error status to ERROR
	ErrorStatus = _tdef_ns::EL_ERROR;
...
void MeanShift::generateLookupTable( void )
{
	...
	for(i = 0; i < kp; i++)
    {
      ...
      // not needed for this kernel --> w[i] = nullptr indicates
			// this
		case _tdef_ns::Uniform:
			
      w        [i] = nullptr;  //weight function not needed for this kernel
...
void MeanShift::CreateBST( void )
{
    ...
    {
		forest[i].x      = &data[i*N];
    forest[i].right  = nullptr;
    forest[i].left   = nullptr;
    forest[i].parent = nullptr;
    }
  ...
  root = BuildKDTree(forest, L, 0, nullptr);
...
/*******************************************************/
/*Pre:                                                 */
/*      - gt is a possibly nullptr pointer to a kd tree   */
  ...
  while (c_t != nullptr)
	{
		switch(actionType) {
		case 0: // forward
      if ((c_t->x[c_d] > range[2*c_d]) && ((c_t->left) != nullptr))
			{
				c_t = c_t->left;
				c_d = (c_d+1)%N;
			} else
      ...
      if ((c_t->x[c_d] < range[2*c_d+1]) && ((c_t->right) != nullptr))
			{
				c_t = c_t->right;
				c_d = (c_d+1)%N;
				actionType = 0;
...
void MeanShift::generalLSearch(double *Mh_ptr, double *yk_ptr)
{
  ...
  int i, j, k;
	...
	//search window bandwidth is specified by
	//h[0]:
  tx = yk_ptr[0] - h[0] + DELTA + 0.99f;
	if (tx < 0)
		LowerBoundX = 0;
	else
		LowerBoundX = (int) tx;
  tx = yk_ptr[1] - h[0] + DELTA + 0.99f;
	if (tx < 0)
		LowerBoundY = 0;
	else
		LowerBoundY = (int) tx;
