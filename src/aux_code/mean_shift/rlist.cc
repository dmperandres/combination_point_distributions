...
RegionList::RegionList(int maxRegions_, int L_, int N_)
{
	...
	if((maxRegions = maxRegions_) <= 0)
		ErrorHandler("RegionList", "Maximum number of regions is zero or negative.", _tdef_ns::FATAL);

	//Obtain dimension of data set being classified by
	//region list class
	if((N = N_) <= 0)
		ErrorHandler("RegionList", "Dimension is zero or negative.", _tdef_ns::FATAL);

	//Obtain length of input data set...
	if((L = L_) <= 0)
		ErrorHandler("RegionList", "Length of data set is zero or negative.", _tdef_ns::FATAL);

	//Allocate memory for index table
	if(!(indexTable = new int [L]))
		ErrorHandler("RegionList", "Not enough memory.", _tdef_ns::FATAL);
...
void RegionList::AddRegion(int label, int pointCount, int *indeces)
{
	...
	if(numRegions >= maxRegions)
		ErrorHandler("AddRegion", "Not enough memory allocated.", _tdef_ns::FATAL);

	//make sure that label is positive and point Count > 0...
	if((label < 0)||(pointCount <= 0))
		ErrorHandler("AddRegion", "Label is negative or number of points in region is invalid.", _tdef_ns::FATAL);

	//make sure that there is enough memory in the indexTable
	//for this region...
	if((freeBlockLoc + pointCount) > L)
		ErrorHandler("AddRegion", "Adding more points than what is contained in data set.", _tdef_ns::FATAL);
