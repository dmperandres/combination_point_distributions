...
{
  _ne_block *Block = new _ne_block(nullptr);
...
//HEA returns the name of the block that is input in port 1
...
_ne_block *_ne_block::get_block_in_port3()
{
  _ne_port *In_port3=in_port3();

  // check if there are any connection
  if (In_port3==nullptr || In_port3->num_conections()==0) return nullptr;
  else{
    // get the vector of connections. There must be only one connection
    QVector<_ne_connection*> Connections=In_port3->connections();
    // get the first one
    _ne_connection *Connection=Connections[0];
    // get the output port of the connection, that is the port1
    _ne_port *Out_port1=Connection->port1();
